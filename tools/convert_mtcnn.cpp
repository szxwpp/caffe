...
// Usage:
//   convert_mtcnn [FLAGS] ROOTFOLDER/ LISTFILE DB_NAME
//
...
//  std::string cmd_str="mkdir " + std::string(argv[3]);
...
//  }
  ...
  if(FLAGS_redo)
  {
      std::string cmd_str = "rm -rf " + std::string(argv[3]);
      system(cmd_str.c_str());
  }
  ...
  Datum* datum = mtcnndatum.mutable_datum();
  int count = 0;
  int data_size = 0;
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    bool status= ReadImageToMTCNNDatum(root_folder + lines[line_id].first,
